...
// downloadChunk acquires a worker token via the scheduler (respecting rate
// limits) and downloads the chunk. On getFile failure it falls back to every
// other worker assigned to the storage.
...
// Each attempt acquires a rate-limited token via the scheduler and includes
...
func (m *StorageManager) downloadChunksInOrderWithLoader(
	...
	return runOrderedJobs(
		...
		func(chunk domain.FileChunk, data []byte) error {
			return writeChunk(chunk, data)
		},
		nil,
	)
}
...
// All chunk downloads go through the scheduler for rate limiting.
